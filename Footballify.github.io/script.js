//Order of Leagues
var desiredOrder = [
  1, 2, 3, 848, 4, 5, 6, 7, 9, 10, 29, 30, 31, 32, 33, 34, 36, 39, 45, 48, 140,
  142, 135, 137, 78, 81, 61, 65, 66, 88, 94, 96, 253, 203, 262, 179, 185, 144,
  188, 169, 11, 13, 40, 41, 42, 43, 235, 207, 218, 141, 136, 333, 307, 197, 62,
  79, 80, 128, 130, 292, 98, 101, 103, 106, 113, 119, 283, 71, 73, 265, 239,
  211, 89,
];
...
var run = async () => {
  const res = await fetch(
    `https://v3.football.api-sports.io/fixtures?date=${isoStr}`,
    {
      headers: {
  ...
  const ordered = desiredOrder.map((id) =>
    json.filter(({ league }) => league?.id === id)
  );
    ...
    for (x = 0; x < filtered[i].length; x++) {
      ...
      if (arrLeagues.includes(filtered[i][x].league.id)) {
        ...
        if (filtered[i][x].league.flag) {
          league.innerHTML =
            filtered[i][x].league.name +
            `<img class='flag' src=${filtered[i][x].league.flag}>`;
        } else {
          league.innerHTML =
            filtered[i][x].league.name +
            `<img class='flag' src=${filtered[i][x].league.logo}>`;
        }
      ...
      var runLineups = async (idParameter) => {
        const lin = await fetch(
          `https://v3.football.api-sports.io/fixtures/lineups?fixture=${idParameter}`,
          {
            headers: {
              "X-RapidAPI-Host": "v3.football.api-sports.io",
              "X-RapidAPI-Key": "e54f3d3972ca8251c1259694b49948de",
        ...
        if (lineupData.length === 0) {
          ...
          if (lineupData[0].formation) {
            ...
            for (let d = 0; d <= lineupData[0].startXI.length - 1; d++) {
              ...
              document
                .querySelector(".sideScoreDiv")
                .appendChild(lineupParentHome);
      ...
      var runStats = async (idParameter) => {
        const stats = await fetch(
          `https://v3.football.api-sports.io/fixtures/statistics?fixture=${idParameter}`,
          {
            headers: {
              "X-RapidAPI-Host": "v3.football.api-sports.io",
              "X-RapidAPI-Key": "e54f3d3972ca8251c1259694b49948de",
      ...
      var runEvents = async (idParameter, home) => {
        const events = await fetch(
          `https://v3.football.api-sports.io/fixtures/events?fixture=${idParameter}`,
          {
            headers: {
              "X-RapidAPI-Host": "v3.football.api-sports.io",
              "X-RapidAPI-Key": "e54f3d3972ca8251c1259694b49948de",
            ...
            if (homeTm == eventsData[d].team.name) {
              ...
              if (eventsData[d].detail == "Normal Goal") {
                if (eventsData[d].assist.name) {
                  eventOccured.innerHTML =
                    `<img class='ball' src="https://www.citypng.com/public/uploads/small/11649467416xzjfid709wwfnn4b4minvcmsdpiyjrajom2djrhvdh5r1fybjfrf2rsp7vq2bc6ujsij9nsb9jfznh2pvoofx8uziapv9ekhjexe.png">` +
                    Number(
                      eventsData[d].time.elapsed + eventsData[d].time.extra
                    ) +
                    "'  " +
                    eventsData[d].player.name +
                    `<h1 class="light"> Assist: ${eventsData[d].assist.name}</h1>`;
                } else {
                  eventOccured.innerHTML =
                    `<img class='ball' src="https://www.citypng.com/public/uploads/small/11649467416xzjfid709wwfnn4b4minvcmsdpiyjrajom2djrhvdh5r1fybjfrf2rsp7vq2bc6ujsij9nsb9jfznh2pvoofx8uziapv9ekhjexe.png">` +
                    Number(
                ...
                if (eventsData[d].comments) {
                  eventOccured.innerHTML =
                    `<svg class="card" width="14" height="13" viewBox="0 0 6 8" fill="#ffa500" style="margin-top: 3.3px;"><title>Yellow Card</title><rect x="0" y="0" width="7" height="8" rx="1" ry="1"></rect></svg>` +
                    Number(
                      eventsData[d].time.elapsed + eventsData[d].time.extra
                    ) +
                    "'  " +
                    eventsData[d].player.name +
                    `<h1 class='light'>${eventsData[d].comments} </h1>`;
                } else {
                  eventOccured.innerHTML =
                    `<svg class="card" width="14" height="13" viewBox="0 0 6 8" fill="#ffa500" style="margin-top: 3.3px;"><title>Yellow Card</title><rect x="0" y="0" width="7" height="8" rx="1" ry="1"></rect></svg>` +
                    Number(
                ...
                eventOccured.innerHTML =
                  `<img class='sub' src="https://media.istockphoto.com/vectors/green-recycle-sign-icon-symbol-on-white-background-triangular-eco-vector-id1209231674?k=20&m=1209231674&s=612x612&w=0&h=OQumbbblFyt8Z2aKRjF2dYeKCsiiLbiR2zi9lW1Jxlg=">` +
                  Number(
                    eventsData[d].time.elapsed + eventsData[d].time.extra
                  ) +
                  "'  " +
                  eventsData[d].player.name +
                  `<h1 class="light"> Out: ${eventsData[d].assist.name}</h1>`;
              } else if (eventsData[d].detail == "Red Card") {
                if (eventsData[d].comments) {
                  eventOccured.innerHTML =
                    `<svg class="card" width="14" height="13" viewBox="0 0 6 8" fill="red" style="margin-top: 3.3px;"><title>Red Card</title><rect x="0" y="0" width="7" height="8" rx="1" ry="1"></rect></svg>` +
                    Number(
                      eventsData[d].time.elapsed + eventsData[d].time.extra
                    ) +
                    "'  " +
              ...
              if (eventsData[d].detail == "Normal Goal") {
                if (eventsData[d].assist.name) {
                  eventOccured.innerHTML =
                    ...
                    "'" +
                    `<img class='ballFloat' src="https://www.citypng.com/public/uploads/small/11649467416xzjfid709wwfnn4b4minvcmsdpiyjrajom2djrhvdh5r1fybjfrf2rsp7vq2bc6ujsij9nsb9jfznh2pvoofx8uziapv9ekhjexe.png">`;
                } else {
                  eventOccured.innerHTML =
                    eventsData[d].player.name +
                    " " +
                ...
                eventOccured.innerHTML =
                  ...
                  "'" +
                  `<svg class="penFloat" width="20" height="20" viewBox="0 0 1024 1024" fill="#52b030" style="min-width: 16px; display: inline-block;"><title>Penalty</title><path d="M328.96 488.928c-100.928 100.96-100.928 265.216 0 366.176s265.216 100.96 366.080 0c100.928-100.96 100.928-265.216 0-366.176-100.896-100.864-265.152-100.864-366.080 0zM653.856 813.728c-62.976 63.008-149.28 75.136-224.576 36.48l16.32-39.232-63.68-63.776-39.392 16.32c-38.592-75.328-26.4-170.176 36.608-233.184 63.008-62.944 149.248-75.168 224.576-36.544l-16.128 38.784 63.744 63.808 38.944-16.192c38.784 75.296 26.72 170.4-36.416 233.536zM526.144 686.144l-21.664 100.16 84.416 32.608 70.048-69.984-33.6-85.408zM369.504 594.528l34.784 82.88 95.712-17.376 17.664-95.36-83.104-35.136zM116.352 162.912v349.088h93.12v-256h605.056v256h93.12v-349.088z"></path></svg>`;
              } else if (eventsData[d].type == "Var") {
                eventOccured.innerHTML =
                  `<h1 class='light'>${eventsData[d].detail} </h1>` +
                  eventsData[d].player.name +
                  ...
                  "'" +
                  `<svg class="varRight" width="18" height="18" viewBox="0 0 32 32"><title>VAR decision</title><path d="M32,4.5H0v20.4h9v2.6h14v-2.6h9V4.5z M30.3,23.1H1.7V6.3h28.5V23.1z"></path><path d="M7.4,17.1l2.3-7.5h2.3l-3.5,10H6.3l-3.5-10h2.3L7.4,17.1z"></path><path d="M17.4,17.5h-3.6l-0.7,2.1H11l3.7-10h1.9l3.7,10h-2.2L17.4,17.5z M14.4,15.9h2.5l-1.3-3.7 L14.4,15.9z"></path><path d="M24.7,15.9H23v3.6H21v-10h3.7c1.2,0,2.1,0.3,2.7,0.8s1,1.3,1,2.2c0,0.7-0.1,1.2-0.4,1.7 c-0.3,0.5-0.7,0.8-1.3,1.1l2.2,4.1v0.1h-2.2L24.7,15.9z M23,14.3h1.7c0.5,0,0.9-0.1,1.2-0.4s0.4-0.6,0.4-1.1c0-0.5-0.1-0.8-0.4-1.1 c-0.3-0.3-0.7-0.4-1.2-0.4H23V14.3z"></path></svg>`;
              } else if (eventsData[d].detail == "Missed Penalty") {
                eventOccured.innerHTML =
                  eventsData[d].player.name +
                  " " +
                  ...
                  "'" +
                  `<svg class="penMissFloat" width="20" height="20" viewBox="0 0 1024 1024" fill="red" style="min-width: 16px; display: inline-block;"><title>Missed Penalty</title><path d="M328.96 488.928c-100.928 100.96-100.928 265.216 0 366.176s265.216 100.96 366.080 0c100.928-100.96 100.928-265.216 0-366.176-100.896-100.864-265.152-100.864-366.080 0zM653.856 813.728c-62.976 63.008-149.28 75.136-224.576 36.48l16.32-39.232-63.68-63.776-39.392 16.32c-38.592-75.328-26.4-170.176 36.608-233.184 63.008-62.944 149.248-75.168 224.576-36.544l-16.128 38.784 63.744 63.808 38.944-16.192c38.784 75.296 26.72 170.4-36.416 233.536zM526.144 686.144l-21.664 100.16 84.416 32.608 70.048-69.984-33.6-85.408zM369.504 594.528l34.784 82.88 95.712-17.376 17.664-95.36-83.104-35.136zM116.352 162.912v349.088h93.12v-256h605.056v256h93.12v-349.088z"></path></svg>`;
              } else if (eventsData[d].detail == "Own Goal") {
                eventOccured.innerHTML =
                  eventsData[d].player.name +
                  " " +
                ...
                if (eventsData[d].comments) {
                  eventOccured.innerHTML =
                    ...
                    "'" +
                    `<svg class="cardFloat" width="14" height="13" viewBox="0 0 6 8" fill="#ffa500" style="margin-top: 3.3px;"><title>Yellow card</title><rect x="0" y="0" width="7" height="8" rx="1" ry="1"></rect></svg>`;
                } else {
                  eventOccured.innerHTML =
                    eventsData[d].player.name +
                    " " +
                ...
                eventOccured.innerHTML =
                  ...
                  "'" +
                  `<img class='subFloat' src="https://media.istockphoto.com/vectors/green-recycle-sign-icon-symbol-on-white-background-triangular-eco-vector-id1209231674?k=20&m=1209231674&s=612x612&w=0&h=OQumbbblFyt8Z2aKRjF2dYeKCsiiLbiR2zi9lW1Jxlg=">`;
              } else if (eventsData[d].detail == "Red Card") {
                if (eventsData[d].comments) {
                  eventOccured.innerHTML =
                    `<h1 class='light'>${eventsData[d].comments} </h1>` +
                    ...
                    "'" +
                    `<svg class="cardFloat" width="14" height="13" viewBox="0 0 6 8" fill="red" style="margin-top: 3.3px;"><title>Red Card</title><rect x="0" y="0" width="7" height="8" rx="1" ry="1"></rect></svg>`;
                } else {
                  eventOccured.innerHTML =
                    eventsData[d].player.name +
                    " " +
      ...
      if (
        gameStatus.innerHTML == "TBD" ||
        gameStatus.innerHTML == "PST" ||
        gameStatus.innerHTML == "NS" ||
        gameStatus.innerHTML == "1H" ||
        gameStatus.innerHTML == "2H" ||
        gameStatus.innerHTML == "ET" ||
        gameStatus.innerHTML == "INT" ||
        gameStatus.innerHTML == "HT"
      ) {
        ...
        //If Home Wins
      } else if (filtered[i][x].teams.home.winner == true) {
        homeTeamName.classList.add("winner");
      ...
      parent.addEventListener("click", function () {
        ...
        homeTeamLogoRightDiv.setAttribute(
          "src",
          homeTeamLogo.getAttribute("src")
        );
        ...
        var x = setInterval(function () {
          ...
          if (Number(seconds) >= 0) {
            ...
            if (
              ...
              if (
                gameStatus.innerHTML == "FT" ||
                gameStatus.innerHTML == "AWD"
              ) {
                timer.innerHTML =
                  homeTeamScoreRightDiv.innerHTML +
                  awayTeamScoreRightDiv.innerHTML;
        ...
        awayTeamLogoRightDiv.setAttribute(
          "src",
          awayTeamLogo.getAttribute("src")
        );
